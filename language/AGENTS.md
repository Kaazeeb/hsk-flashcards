...
- `grammar_vocabulary_exceptions.csv` is limited to explicitly authorized official-grammar inventory surfaces that have no standalone syllabus vocabulary sense at or below the grammar point's introduction level. An approved row applies only to reviewed grammar examples for its exact `grammar_point_id`, level, surface, and target role. It creates no `vocab_id`, cannot map or relabel a later-level sense, and must not become a general path for proper names, unrelated above-level vocabulary, or ordinary lexical variants.
